...
class Movie(Core):
    ...
    def get_alternative_titles(self):
        alternatives = self.getJSON(config['urls']['movie.alternativetitles'] % self.movie_id)
        result = [ ]
        for alternative in alternatives["titles"]:
            result.append({ 'lang' : alternative['iso_3166_1'], 'title' : alternative['title'] })
        return result
